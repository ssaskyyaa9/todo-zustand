...
export default function DashboardScreen() {
  ...
  const todoItems = notes.filter( (item) => item.type === "todo" );
  const noteItems = notes.filter( (item) => item.type !== "todo" );
  const remainingTodoCount = todoItems.filter( (item) => !item.completed ).length;
  ...
  const loadNotes = async () => {
    try {
      ...
      if (!user) { router.replace("/auth/AuthScreen"); return; }
  ...
  const handleToggle = async ( id, currentStatus ) => { const nextStatus = !currentStatus;
    const updatedNotes = notes.map((item) =>
      item.id === id ? { ...item, completed: nextStatus, } : item
    );
    ...
    if (error) {
      console.log( "Gagal memperbarui status:", error.message );
  ...
  const renderTodoItem = ({ item }) => {
    ...
    return (
      <View style={{ backgroundColor: theme.cardBg, borderRadius: 17, padding: 14, marginBottom: 10, flexDirection: "row", alignItems: "center", borderWidth: 1, borderColor: theme.cardBorder, }} >
        <TouchableOpacity activeOpacity={0.7} onPress={() => handleToggle( item.id, completed ) } style={{ paddingHorizontal: 6, paddingVertical: 4, }} >
          {completed ? (
            <CheckSquare size={22} color={theme.todoColor} strokeWidth={2.2} />
          ) : (
        ...
        >
          <Text numberOfLines={1} style={{ fontSize: 15, fontWeight: "700", color: theme.textPrimary, marginBottom: 4, textDecorationLine: completed ? "line-through" : "none", opacity: completed ? 0.5 : 1, }} > {item.title} </Text>
  ...
  const renderNoteItem = ({ item }) => {
    return (
      <TouchableOpacity activeOpacity={0.8}
        onPress={() => router.push( `/screen/detailScreen?id=${item.id}` ) }
        style={{ backgroundColor: theme.cardBg, borderRadius: 17, padding: 16, marginBottom: 10, borderWidth: 1, borderColor: theme.cardBorder, }}
      >
        <View style={{ flexDirection: "row", alignItems: "flex-start", }} >
          <View style={{ width: 42, height: 42, borderRadius: 13, backgroundColor: isDarkMode ? "#26384A" : "#EAF7FF", alignItems: "center", justifyContent: "center", }} >
            <FileText size={20} color={theme.primary} strokeWidth={2} />
          </View>

          <View style={{ flex: 1, marginLeft: 12, }} >
            <Text numberOfLines={1} style={{ fontSize: 15, fontWeight: "700", color: theme.textPrimary, marginBottom: 5, }} > {item.title} </Text>
            <Text numberOfLines={2} style={{ fontSize: 12, lineHeight: 18, color: theme.textSecondary, marginBottom: 7, }} > {item.content} </Text>
            <View style={{ flexDirection: "row", alignItems: "center", }} >
              <Clock3 size={12} color={theme.textSecondary} strokeWidth={2} />
              <Text style={{ marginLeft: 5, fontSize: 11, color: theme.textSecondary, }} >
                {formatDate(item.created_at)}
  ...
  const ListHeader = () => (
    <>
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 28, }} >
        ...
        <TouchableOpacity activeOpacity={0.8} onPress={toggleTheme} style={{ width: 46, height: 46, borderRadius: 14, backgroundColor: theme.cardBg, alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: theme.cardBorder, }} >
          {isDarkMode ? (
      ...
      <View style={{ backgroundColor: theme.primary, borderRadius: 24, padding: 22, marginBottom: 22, overflow: "hidden", }} >
        ...
        <Text style={{ color: "#FFFFFF", fontSize: 38, fontWeight: "800", marginBottom: 4, }} > {loading ? "..." : remainingTodoCount} </Text>
        <Text style={{ color: "#DCEFFF", fontSize: 14, }} > {totalTodoCount} Todo · {totalNoteCount} Note </Text>


        <TouchableOpacity
          activeOpacity={0.8}
          onPress={() => router.push( "/screen/addnoteScreen" ) }
          style={{ marginTop: 20, alignSelf: "flex-start", flexDirection: "row", alignItems: "center", backgroundColor: "#FFFFFF", paddingHorizontal: 16, paddingVertical: 11, borderRadius: 12, }}
        >
          <Plus size={17} color={theme.primary} strokeWidth={2.5} />
          <Text style={{ marginLeft: 7, color: theme.primary, fontWeight: "800", fontSize: 13, }} > Tambah </Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 14, }} >
        <View>
          ...
          <Text style={{ fontSize: 12, color: theme.textSecondary, marginTop: 3, }} > {remainingTodoCount} tugas belum selesai </Text>
        </View>
      </View>
    </>
  );
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.bg, }} >
        ...
        ListEmptyComponent={
          loading ? (
            ...
            <View style={{ backgroundColor: theme.cardBg, borderRadius: 17, padding: 30, alignItems: "center", borderWidth: 1, borderColor: theme.cardBorder, }} >
              ...
              <Text style={{ fontSize: 16, fontWeight: "800", color: theme.textPrimary, marginBottom: 5, }} > Belum ada data </Text>

              <Text style={{ fontSize: 13, color: theme.textSecondary, textAlign: "center", marginBottom: 18, }} > Buat catatan atau todo pertama kamu. </Text>

              <TouchableOpacity activeOpacity={0.8}
                onPress={() =>
                  router.push( "/screen/addnoteScreen" )
                }
                style={{ flexDirection: "row", alignItems: "center", backgroundColor: theme.primary, paddingHorizontal: 17, paddingVertical: 11, borderRadius: 12, }}
              >
                <Plus size={17} color="#FFFFFF" strokeWidth={2.5} />
                <Text style={{ color: "#FFFFFF", fontSize: 13, fontWeight: "800", marginLeft: 7, }} > Buat Baru </Text>
              </TouchableOpacity>
            </View>
          )
        }
      ...
      <View style={{ position: "absolute", left: 16, right: 16, bottom: 18, height: 68, backgroundColor: theme.cardBg, borderRadius: 20, flexDirection: "row", alignItems: "center", justifyContent: "space-around", borderWidth: 1, borderColor: theme.cardBorder, elevation: 5, }} >
        <TouchableOpacity activeOpacity={0.7} style={{ alignItems: "center", justifyContent: "center", }} > <FileText size={21} color={theme.primary} strokeWidth={2.2} />
